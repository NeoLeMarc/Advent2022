...
for s in dirsum:
    ...
    if str(s[0]) == '/':
        rootsize = s[1]
print(sum)
print("Rootsize: %i" % rootsize)


# Now we need to find the smallest directory to delete
sizelimit = 30000000 - (70000000 - rootsize)
print("Required: %i" % sizelimit)

candidates = [c for c in dirsum if c[1] >= sizelimit]
print(candidates)

cand = [c[1] for c in candidates]
print(min(cand))
